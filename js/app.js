...
function renderTracker(el) {
  ...
  function refreshHistory() {
    ...
    const prev = state.sessions.filter(s => selectedCalDate || s.date !== todayISO())
...
function renderInsights(el) {
  ...
  async function fetchAI() {
    ...
    const cacheKey = JSON.stringify(sessions.map(s => s.id)).slice(0, 80);
    ...
    const summary = `Data: ${sessions.length} sesi. Subjek: ${[...new Set(sessions.map(s => subjById(s.subjectId).name))].join(', ')}. Avg pemahaman: ${sessions.length ? Math.round(sessions.reduce((a, s) => a + s.understanding, 0) / sessions.length * 10) / 10 : 0}/10. Total waktu: ${fmtMin(sessions.reduce((a, s) => a + s.duration, 0))}. Streak: ${computeStreak(sessions)} hari. Burnout: ${computeBurnoutScore(sessions)}%. Per subjek: ${state.subjects.map(s => { const ss = sessions.filter(x => x.subjectId === s.id); if (!ss.length) return null; const avg = Math.round(ss.reduce((a, x) => a + x.understanding, 0) / ss.length * 10) / 10; return `${s.name}: ${ss.length} sesi, avg ${avg}/10, total ${fmtMin(ss.reduce((a, x) => a + x.duration, 0))}`; }).filter(Boolean).join('; ')}.`;
    const systemPrompt = 'Kamu adalah asisten learning analytics. Analisis data belajar mahasiswa dan berikan insight singkat dalam bahasa Indonesia. Format: 3-4 paragraf singkat (maks 200 kata), pisahkan setiap paragraf dengan baris kosong (double newline). Boleh menebalkan istilah penting dengan **teks**. Jangan gunakan bullet point atau heading. Fokus pada: (1) pola belajar terdeteksi, (2) risiko burnout, (3) satu rekomendasi konkret. Nada: supportif, to-the-point, berbasis data.';
    const userPrompt = `Data belajar:\n${summary}\n\nBerikan analisis singkat dan 1 rekomendasi terpenting.`;
    try {
